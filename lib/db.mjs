...
moduleExports.handleQuery = function (settings, queryHandler) {
  let pgClient;
...
/**
 * Read content from postgresql table table
 *
 * @param {object} settings - global settings
 * @param {string} schemaName - name of databse schema
 * @param {string} tableName - name of table in database schema
 *
 * @returns {Promise} Promise with result of query
 */
moduleExports.readTableContent = function (settings, schemaName, tableName) {
  return moduleExports
    .handleQuery(settings, (pgClient) => {
      logger.debug(`Read content from table ${schemaName}.${tableName}`);
      return pgClient.query({
        text: `SELECT * FROM ${schemaName}.${tableName}`,
      });
    })
    .then((result) => result.rows);
};
